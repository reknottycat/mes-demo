...
- `--signal-blue: #1589EE` — neutral system information, gateway connectivity, loading and traceability.
- `--signal-cyan: #2ACCC3` — help or explanatory guidance; never used for production state.
...
- Runtime token mapping: `--signal-orange → --orange`, `--signal-blue → --blue`, `--signal-cyan → --cyan`, `--signal-green → --green`, `--signal-red → --red`, and `--signal-yellow → --yellow` in `app/globals.css`. The deep/surface tokens keep their documented CSS names.
...
- State palette: blue = system information/connectivity; cyan = help; green = normal/complete; orange = current production tempo; yellow = waiting/attention; red = ANDON/fault/blockage. All state tones must have text labels; no critical status is color-only.
